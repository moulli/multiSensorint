...
%  NB: DATA CAN BE NORMALIZED BEFORE REGRESSION!
%
%
%% Parameters:
%
%  --h5file: HDF5 file or link to HDF5 file. The file must have a
%    '/Data/Stimulus' path, a '/Data/Brain/DFF' path, and a
%    '/Data/Brain/Times' path. Stimulus must be a (1 x time) array, DFF a 
...
%         decay convolution.
%       --Params.fprint (default: 10000): print progress after Params.fprint 
%         iterations done.
...
%  --perinf: cell with retained period, mean stimulus and mean signal.
    ...
    % Getting files:
    stim_path = h5readatt(h5file, '/Metadata', 'Stimulus path');
    stim = h5read(h5file, stim_path);
    time = h5read(h5file, '/Data/Brain/Times');
    dff = h5read(h5file, '/Data/Brain/Analysis/DFF');
    % Getting dimensions:
    [nneu, ntime] = size(dff);
    ...
    % Normalizing data:
    if ~isfield(Params, 'dffnorm') || Params.dffnorm == "none"
        dffn = dff;
    ...
    if ~isfield(Params, 'fprint'); Params.fprint = 10000; end
    % Defining output:
    out = struct;
